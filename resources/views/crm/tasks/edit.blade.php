@extends('layouts.vertical', ['page_title' => 'Upravit úkol'])
...
<div class="container-fluid">
  <div class="page-title-box">
    ...
    <h4 class="page-title">Upravit úkol</h4>
  </div>
  ...
  <div class="card">
    <div class="card-body">
      <form method="POST" action="{{ route('tasks.update', $task) }}">
        ...
        <div class="row g-3">
          ...
          <div class="col-md-6">
            ...
            <select name="type" class="form-select">
              @foreach(['call','email','meeting','follow_up','proposal','other'] as $t)
                <option value="{{ $t }}" @selected(old('type',$task->type)===$t)>{{ ucfirst(str_replace('_',' ',$t)) }}</option>
              @endforeach
            </select>
          </div>
          <div class="col-md-4">
          ...
          <div class="col-md-4">
            ...
            <select name="priority" class="form-select">
              @foreach(['low','medium','high','urgent'] as $p)
                <option value="{{ $p }}" @selected(old('priority',$task->priority)===$p)>{{ ucfirst(str_replace('_',' ',$p)) }}</option>
              @endforeach
            </select>
          </div>
          <div class="col-md-4">
            <label class="form-label">Termín</label>
            <input type="datetime-local" name="due_date" class="form-control" value="{{ old('due_date', optional($task->due_date)->format('Y-m-d\TH:i')) }}" required />
          </div>
          <div class="col-md-6">
          ...
          <div class="col-12">
            ...
            <div class="input-group">
              <select id="rel-type" name="taskable_type" class="form-select" style="max-width:220px">
                <option value="" @selected(empty(old('taskable_type', $taskableKey)))>—</option>
                <option value="company" @selected(old('taskable_type', $taskableKey)==='company')>Společnost</option>
                <option value="contact" @selected(old('taskable_type', $taskableKey)==='contact')>Kontakt</option>
                <option value="lead" @selected(old('taskable_type', $taskableKey)==='lead')>Lead</option>
                <option value="opportunity" @selected(old('taskable_type', $taskableKey)==='opportunity')>Příležitost</option>
                <option value="project" @selected(old('taskable_type', $taskableKey)==='project')>Projekt</option>
              </select>
              <input type="hidden" name="taskable_id" id="rel-id" value="{{ old('taskable_id', $task->taskable_id) }}">
        ...
        <div class="mt-3">
          <a href="{{ route('tasks.show', $task) }}" class="btn btn-light">Zpět</a>
